...
export default function VerifierActions({
  appId,
  headers,
  onDone,
  disabled = false,
  ...
  const approveDisabled = disabled || appStatus === "RETURNED";
...
const { refreshCounts } = useOutletContext();
  ...
  const presetMap = useMemo(() => {
    return {
      ...
      RETURN: [
        { key: "need-more-doc", label: "ຕ້ອງເພີ່ມເອກະສານ", text: "ກະລຸນາເພີ່ມເອກະສານທີ່ຂາດໃຫ້ຄົບ" },
        { key: "fix-data", label: "ແກ້ໄຂຂໍ້ມູນ", text: "ກະລຸນາກວດແກ້ຂໍ້ມູນໃຫ້ຖືກຕ້ອງ (ຊື່/ເລກບັດ/ທີ່ຢູ່)" },
        { key: "reupload", label: "ອັບໂຫຼດໃໝ່", text: "ກະລຸນາອັບໂຫຼດໄຟລ໌/ຮູບເອກະສານໃໝ່ ໃຫ້ຊັດເຈນ" },
      ],
    };
  }, []);
  ...
  const meta = useMemo(() => {
    ...
    if (action === "RETURN") {
      return {
        title: "ສົ່ງກັບໃຫ້ແກ້ (Return / Need Fix)",
        desc: "ຈະສົ່ງກັບໃຫ້ Credit Officer ແກ້ໄຂຂໍ້ມູນ/ເອກະສານ",
        confirmText: "ສົ່ງກັບ",
        requireComment: true,
        variant: "secondary",
        endpoint: endpoints?.return,
      };
    }
    if (action === "REJECT") {
      return {
