...
const DCOLayout = () => {
  ...
  // ✅ ใช้ context เดียวกันทั้งระบบ (Verifier/DCO/CEO ได้)
  const { counts, setCounts } = useCounts();

  // ✅ refreshCounts: นับเคสที่รอ DCO
  const refreshCounts = async () => {
    try {
      ...
      const res = await axios.get(`${Url.base_url}/loan-applications/by-status`, {
        params: { status: "PENDING_DCO", page: 1, limit: 1 },
      ...
      if (res.data?.success) {
        setCounts((prev) => ({
          ...prev,
          pendingDco: res.data.pagination?.total || 0,
        }));
      }
    } catch (e) {
      console.error("refreshCounts (DCO) error:", e);
    }
  };
  ...
  const navItems = useMemo(
    () => [
...

      {
        ...
        group: "DCO Review",
        badge: counts?.pendingDco > 0 ? counts.pendingDco : null,
      },

      { to: `${BASE}/all-cases`, icon: FileSearch, label: "All Cases", group: "DCO Review" },
          ...
          <div className="flex items-center gap-3 px-4 sm:px-6 py-3">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="outline">DCO</Button>
              </DropdownMenuTrigger>
