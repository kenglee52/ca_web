...
const VerifierLayout = () => {
  ...
  const navItems = useMemo(
    () => [
...

      {
        to: `${BASE}/inbox`,
        icon: Inbox,
        label: "Inbox (To Verify)",
        group: "Verification",
        badge: counts.pendingVerifier > 0 ? counts.pendingVerifier : null,
       
      
      },

      { to: `${BASE}/cases`, icon: FileSearch, label: "All Cases", group: "Verification" },
      { to: `${BASE}/approved`, icon: FileCheck2, label: "Approved", group: "Status" },
      { to: `${BASE}/history`, icon: History, label: "My Activity", group: "Audit" },
 
    ],
    [counts.pendingVerifier] // ✅ สำคัญมาก
  );
  ...
  const refreshCounts = async () => {
    try {
      ...
      const res = await axios.get(`${Url.base_url}/loan-applications/by-status`, {
        params: { status: "PENDING_VERIFIER", page: 1, limit: 1 },
        headers: { Authorization: `Bearer ${token}` },
      });

      if (res.data?.success) {
        setCounts({ pendingVerifier: res.data.pagination?.total || 0 });
      }
    } catch (e) {
 ...
 useEffect(() => {
    ...
    <aside className="w-72 h-full bg-slate-950 text-slate-200 flex flex-col p-4 shadow-2xl">
      ...
      <div className="flex items-center gap-3 px-2 py-3 mb-2">
        ...
        <div className="leading-tight">
          ...
          <div className="text-xs text-slate-400">Document Verification Portal</div>
        </div>
      </div>
            ...
            <div className="space-y-1">
              {items.map((item) => (
                <NavLink
                  ...
                  onClick={() => { setIsSidebarOpen(false);  refreshCounts(); }}
                  className={({ isActive }) =>
                    [
  ...
  return (
    // ✅ Layout คงที่ (ไม่เลื่อนทั้งหน้า)
          ...
          <div className="flex items-center gap-3 px-4 sm:px-6 py-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-xs text-slate-500 truncate">
                Verify documents, return for fixes, and approve cases
              </div>
            </div>
