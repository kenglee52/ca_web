// src/pages/verifier/VerifierInbox.jsx
...
const fmtMoney = (v) => {
  ...
  return Number(v).toLocaleString("lo-LA");
};
...
const VerifierInbox = () => {
  ...
  const fetchApplications = async () => {
    ...
    try {
      ...
      toast.error("ບໍ່ສາມາດດຶງລາຍການຄຳຮ້ອງຂໍສິນເຊື່ອໄດ້");
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  const getStatusBadge = (status) => {
    if (status === "PENDING_VERIFIER") {
      return <Badge variant="default" className="bg-blue-500 hover:bg-blue-600 text-white px-3 py-1">ລໍຖ້າການກວດສອບ</Badge>;
    }
    if (status === "RETURNED") {
      return <Badge variant="destructive" className="bg-red-500 hover:bg-red-600 text-white px-3 py-1">ລໍຖ້າການແກ້ໄຂ</Badge>;
    }
    return <Badge variant="outline" className="px-3 py-1">{status || "ບໍ່ລະບຸ"}</Badge>;
  };
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-orange-700">ກ່ອງຂໍ້ຄວາມຜູ້ກວດສອບ</h1>
          <p className="text-muted-foreground mt-1">
            ລາຍການຄຳຂໍກູ້ທີ່ລໍຖ້າການກວດສອບ (PENDING_VERIFIER)
          </p>
        </div>
      ...
      <Card className="border-none shadow-lg">
        <CardHeader>
          <CardTitle className="text-lg">ຄຳຂໍກູ້ທີ່ລໍຖ້າການກວດສອບ (PENDING_VERIFIER)</CardTitle>
        </CardHeader>
        ...
        <CardContent>
          {loading ? (
            ...
            <div className="text-center py-12 text-muted-foreground">
              ບໍ່ພົບຄຳຂໍກູ້ທີ່ລໍຖ້າການກວດສອບໃນສະຖານະ PENDING_VERIFIER
            </div>
          ) : (
            <>
                ...
                <Table>
                  <TableHeader className="bg-gray-50">
                    <TableRow>
                      ...
                      <TableHead>ສະຖານະ</TableHead> {/* เพิ่มคอลัมน์สถานะ */}
...
// ฟังก์ชันช่วยแสดงสถานะภาษาลาว + Badge สี
const getStatusBadge = (status) => {
  if (status === "PENDING_VERIFIER") {
    return <Badge variant="default" className="bg-blue-500 hover:bg-blue-600 text-white px-3 py-1">ລໍຖ້າການກວດສອບ</Badge>;
  }
