// src/pages/verifier/VerifierReturned.jsx
...
import { Search, Eye, RefreshCw, AlertTriangle } from "lucide-react";
...
const VerifierReturned = () => {
  ...
  const fetchReturned = async () => {
    ...
    try {
      ...
      const res = await axios.get(`${Url.base_url}/loan-applications/by-status`, {
        params: {
          status: "RETURNED",  // ล็อกให้ดึงเฉพาะ RETURNED
          q: search || undefined,
          page,
          limit,
        },
      ...
      toast.error("ບໍ່ສາມາດດຶງຄຳຂໍທີ່ຖືກສົ່ງກັບໄດ້");
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  const handleViewDetail = (id) => {
    navigate(`/verifier/applications/${id}`);
  };
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-orange-700">ຄຳຂໍທີ່ຖືກສົ່ງກັບແກ້ໄຂ</h1>
          <p className="text-muted-foreground mt-1">
            ລາຍການຄຳຂໍກູ້ທີ່ Verifier ສົ່ງກັບໃຫ້ CO ແກ້ໄຂ (RETURNED)
          </p>
        </div>
        <Button onClick={fetchReturned} variant="outline" className="gap-2">
        ...
        <CardContent>
          {loading ? (
            ...
            <div className="text-center py-12 text-muted-foreground">
              ບໍ່ພົບຄຳຂໍກູ້ທີ່ຖືກສົ່ງກັບແກ້ໄຂ
            </div>
          ) : (
            <>
                ...
                <Table>
                  ...
                  <TableBody>
                    {applications.map((app) => (
                      <TableRow key={app.id} className="hover:bg-red-50/30 transition-colors">
                        <TableCell>
                          {app.submittedAt
...
export default VerifierReturned;